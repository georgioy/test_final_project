...
require_once('../../DAL/DAL.class.php');
   ...
   class customer{
    ...
    public function addCustomer($username, $email, $password, $address, $gender, $status){
        $sql = "insert into customers (id, username, email, password, address, gender, status) values('', '$username', '$email', '$password', '$address', '$gender', '$status')";
        $db = new DAL();
        $rows = $db->ExecuteQuery($sql);
        return $rows;
    }
